...
#PARAMETERS ###########################
abund_table<-read.csv("../../data/VSEARCH/pathway_abundance.tsv",sep="\t",header=T,row.names=1,check.names=FALSE)
#abund_table<-read.csv("../../data/ko_metagenome.tsv",sep="\t",header=T,row.names=1,check.names=FALSE)
...
#Rounding up is required for pathways
...
#At this point we have abund_table, meta_table are ready and their dimensions should match
#/DATA IMPORT############################################################
...
write.csv(data_to_write,paste("Diversity_",label,".csv",sep=""))
...
pdf(paste("ANOVA_diversity_",label,".pdf",sep=""),height=height_image,width=width_image)
print(p)
dev.off()
