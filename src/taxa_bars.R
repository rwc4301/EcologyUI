...
run_analysis <- function(abund_table, meta_table, label) {
...
# meta_table$Groups <- sapply(strsplit(rownames(meta_table), "\\."), `[`, 1)
meta_table$Groups <- paste("Run ", meta_table$run, "Week ", meta_table$week)

# All
# label <- "All"
# run_analysis(abund_table, meta_table, label)
# 
# # Split by beds
# for (x in 1:10) {
#   label <- paste("Bed ", x)
#   sub_meta_table <- meta_table[meta_table$BED == x,]
#   run_analysis(abund_table, sub_meta_table, label)
# }
# 
# # Split by week
# for (x in 1:9) {
#   label <- paste("Week ", x)
#   sub_meta_table <- meta_table[meta_table$week == x,]
#   run_analysis(abund_table, sub_meta_table, label)
# }

# Flow
label <- paste("Flow 100")
sub_meta_table <- meta_table[meta_table$sweetening.flow == "100",]
run_analysis(abund_table, sub_meta_table, label)

label <- paste("Flow 60")
sub_meta_table <- meta_table[meta_table$sweetening.flow == "60",]
run_analysis(abund_table, sub_meta_table, label)

label <- paste("Flow 30")
sub_meta_table <- meta_table[meta_table$sweetening.flow == "30",]
run_analysis(abund_table, sub_meta_table, label)

label <- paste("Flow DS")
sub_meta_table <- meta_table[meta_table$sweetening.flow == "DS",]
run_analysis(abund_table, sub_meta_table, label)

label <- paste("Flow Ext. DS")
sub_meta_table <- meta_table[meta_table$sweetening.flow == "Ext. DS",]
run_analysis(abund_table, sub_meta_table, label)
